...
int main(int argc, char *argv[]) {
    ...
    ctx->initialize();
    [[maybe_unused]] SDL_Window *window = ctx->create_window("Animation Example", kWidth, kHeight);
    ...
    for (auto i = 0; i < astronaut_model.meshes().size(); ++i) {
        // copy data
        meshes_data.push_back(astronaut_model.meshes().at(i).data());
        // vertices
        const VkDeviceSize v_buf_size = sizeof(Vertex) * meshes_data.back().vertices_.size();
        ...
        vert_buffers.back().update(meshes_data.back().vertices_.data());
        // indices
        const VkDeviceSize i_buf_size = sizeof(uint32_t) * meshes_data.back().indices_.size();
        ...
        index_buffers.back().update(meshes_data.back().indices_.data());
    }
    ...
    while (!quit) {
        ...
        {
            // upload view and projection to buffer
            ...
            ctx->begin_rendering(scene_pass, frame_buffer);
            {
                ...
                for (auto i = 0; i < astronaut_model.meshes().size(); ++i) {
                    ...
                    ctx->draw_indexed(meshes_data.at(i).indices_.size());
                }
            }
            ctx->end_rendering();
        }
    ...
    astronaut_model.destroy_textures();
